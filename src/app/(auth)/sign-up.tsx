...
export default function SignUpScreen() {
  ...
  const validateForm = () => {
    ...
    // Terms agreement
    if (!agreeToTerms) {
      showAlert('Terms Required', 'Please agree to the Terms of Service and Privacy Policy to continue.', 'warning')
      isValid = false
    }
    ...
    if (error) {
      console.error('Sign up error:', error)
        ...
        if (error.message.includes('User already registered')) {
          showAlert('Account Exists', 'An account with this email already exists. Please sign in instead.', 'warning')
        } else if (error.message.includes('Password should be at least')) {
          showAlert('Weak Password', 'Please choose a stronger password with at least 8 characters.', 'error')
        } else {
          showAlert('Sign Up Error', error.message, 'error')
        }
      } else if (data.user) {
        console.log('User created successfully:', data.user.id)
        showAlert('Account Created!', 'Welcome to FitMeal! You can now sign in with your credentials.', 'success')
        ...
        >
          ...
          <View className="px-6">
            ...
            <View className="mb-4">
              ...
              <View className={`flex-row items-center bg-gray-50 rounded-xl px-4 py-4 ${nameError ? 'border border-red-300' : ''}`}>
        ...
        <TextInput
          ...
          value={password}
                ...
                <View className="mt-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <View className="space-y-2">
                    <View className="flex-row items-center">
                      ...
                      />
                      <AppText className={`text-xs ml-2 ${passwordValidation.length ? 'text-green-600' : 'text-gray-500'}`}>
                    ...
                    <View className="flex-row items-center">
                      ...
                      />
                      <AppText className={`text-xs ml-2 ${passwordValidation.uppercase ? 'text-green-600' : 'text-gray-500'}`}>
                    ...
                    <View className="flex-row items-center">
                      ...
                      />
                      <AppText className={`text-xs ml-2 ${passwordValidation.lowercase ? 'text-green-600' : 'text-gray-500'}`}>
                    ...
                    <View className="flex-row items-center">
                      ...
                      />
                      <AppText className={`text-xs ml-2 ${passwordValidation.number ? 'text-green-600' : 'text-gray-500'}`}>
                        One number
                      </AppText>
                    </View>
                  </View>
            ...
            <View className="mb-4">
              ...
              <View className={`flex-row items-center bg-gray-50 rounded-xl px-4 py-4 ${confirmPasswordError ? 'border border-red-300' : ''}`}>
                <View className="w-10 h-10 bg-orange-100 rounded-full items-center justify-center mr-3">
                  <Ionicons name="lock-closed-outline" size={20} color="#FB923C" />
                </View>
                <TextInput
            ...
            <View className="mb-6">
              <TouchableOpacity
                ...
                disabled={loading}
              >
                <View className={`w-5 h-5 rounded border-2 mr-3 mt-0.5 items-center justify-center ${agreeToTerms ? 'bg-orange-500 border-orange-500' : 'border-gray-300'}`}>
                  {agreeToTerms && <Ionicons name="checkmark" size={14} color="#FFFFFF" />}
                </View>
                <AppText className="flex-1 text-sm text-gray-600 leading-5">
                  I agree to the{' '}
                  <AppText className="text-orange-500 font-medium">Terms of Service</AppText>
                  {' '}and{' '}
                  <AppText className="text-orange-500 font-medium">Privacy Policy</AppText>
                </AppText>
              </TouchableOpacity>
      </View>
